...
if (($this->state->get('filter.age_category') != 'All' &&
	...
	{ 
		$this->display_position_ind = TRUE;
		$this->columns = 7;
	} else {
		$this->display_position_ind = FALSE;
		$this->columns = 6;
	}
?>
...
<form action="<?php echo JRoute::_('index.php?option=com_rankings'); ?>" method="post" name="AdminForm" id="AdminForm">
    ...
    <table id="tt-table-rankings" class="table-hover tt-table">
		<thead>
			<tr>
				<?php if ($this->display_position_ind)
				{ ?>
		...
		<tbody>
    		...
    		{
    			$this->_rankingView->ranking 				= $this->rankings[$i];
    			$this->_rankingView->row_number 			= $i+1;
    			$this->_rankingView->display_position_ind 	= $this->display_position_ind;
    			$this->_rankingView->state 					= $this->state;
		        echo $this->_rankingView->render();
    			?>
						...
						<div class="tt-rides-box">
							...
							<table class="table-hover tt-table tt-rankings-rides">
								...
								<tbody>
    								...
    								{
    									$this->_rideView->ride = $this->rankings[$i]->rides[$j];
		        						echo $this->_rideView->render();
    								} ?>
								</tbody>
							</table>
						</div>
